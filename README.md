...
**Send as few HTTP headers as possible.** This web server has a small header buffer and drops the connection rather than answering when sent a browser-sized header block. Adding `Origin`, `Accept`, `Accept-Language`, `Cache-Control`, `Upgrade-Insecure-Requests` and an explicit `User-Agent` produced a disconnect on every request, while a minimal POST to the same endpoint returned HTTP 200. The integration sends `Referer` and nothing else. If you are debugging with `curl` or a script and getting empty responses, strip your headers before assuming the switch or the credentials are at fault.
